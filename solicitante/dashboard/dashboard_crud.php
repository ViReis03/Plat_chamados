...
if ($_POST['type'] == 'aberto') {
  ...
  try {
    $sql = $conn->query("
      ...
      where status_ticket = 1 and g.codigo_usuario = $usuario
    ");
    $dashcount = 0;
    foreach ($sql as $row) {
...
if ($_POST['type'] == 'atendimento') {
  ...
  try {
    $sql = $conn->query("
      ...
      from geralticket as g inner join usuario as u
      ON g.codigo_usuario = u.codigo_usuario
      where status_ticket = 2 and g.codigo_usuario = $usuario
    ");
    $dashcount = 0;
    foreach ($sql as $row) {
      if ($row['codigo_ticket'] != "") {
...
if ($_POST['type'] == 'aguardando') {
  ...
  try {
    $sql = $conn->query("
      ...
      from geralticket as g inner join usuario as u
      ON g.codigo_usuario = u.codigo_usuario
      where status_ticket = 3 and g.codigo_usuario = $usuario
    ");
    $dashcount = 0;
    foreach ($sql as $row) {
      if ($row['codigo_ticket'] != "") {
...
if ($_POST['type'] == 'finalizado') {
  ...
  try {
    $sql = $conn->query("
      ...
      from geralticket as g inner join usuario as u
      ON g.codigo_usuario = u.codigo_usuario
      where status_ticket = 4 and g.codigo_usuario = $usuario
    ");
    $dashcount = 0;
    foreach ($sql as $row) {
      if ($row['codigo_ticket'] != "") {
